  ...
  if (featuresEl) {
    featuresEl.innerHTML = c.features.map(f => `
      <div class="about-feature">
        ...
        <p class="about-feature-title">${escapeHtml(f.title)}</p>
      </div>
    `).join('');
  }
}
...
function renderPick(p) {
  toggleSection('#pick', p.visible);
  const grid = document.querySelector('.pick-grid');
  if (!grid) return;

  const items = (p.items || []).filter(item => item.visible !== false);
  if (!items.length) {
    toggleSection('#pick', false);
    return;
  }
  ...
  grid.innerHTML = items.map(item => {
    ...
    const attrs = item.url
      ...
      : '';
    return `
    <${tag} class="pick-card fade-in${item.url ? ' pick-card-link' : ''}" ${attrs}>
      <div class="pick-card-accent type-${escapeAttr(item.type)}"></div>
      <div class="pick-card-body">
        <span class="pick-badge type-${escapeAttr(item.type)}">${escapeHtml(item.badge)}</span>
        <h3 class="pick-card-title">${escapeHtml(item.title)}</h3>
        <p class="pick-card-desc">${escapeHtml(item.desc)}</p>
        ${item.detail ? `<p class="pick-card-detail">${escapeHtml(item.detail)}</p>` : ''}
        ${item.period ? `<p class="pick-card-period">${escapeHtml(item.period)}</p>` : ''}
        ${item.url ? `<p class="pick-card-cta">詳しくはこちら →</p>` : ''}
      </div>
    </${tag}>
  `;
  }).join('');
...
function renderAccess(a) {
  ...
  if (mapEl) {
    const src = a.mapEmbedUrl
      ...
      : `https://maps.google.com/maps?q=${encodeURIComponent(a.address || a.area)}&hl=ja&z=17&output=embed`;
...
function renderContact(c) {
  ...
  if (igLink && c.instagramUrl) igLink.href = c.instagramUrl;

  const noteEl = document.querySelector('.contact-note');
  if (noteEl && c.note) noteEl.textContent = c.note;
}
